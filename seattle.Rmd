...
SEA_elections_odd <- readRDS("data/seattle/SEA_elections_odd.rds") %>% 
  mutate(area_miles = set_units(st_area(geometry), mi^2)) # local races
SEA_elections_even <- readRDS("data/seattle/SEA_elections_even.rds") %>% 
  st_as_sf() %>% 
  st_make_valid() %>% 
  mutate(area_miles = set_units(st_area(geometry), mi^2)) # federal races

SEA_lanes_final <- readRDS("data/seattle/SEA_lanes_final.rds") %>% 
  mutate(year_inter = ifelse(is.na(year_inter), "-", year_inter))

SEA_lanes_final %>% 
  select(streetname, type, official_complete, INSTALL_YR, year_before, year_inter, year_after, link, before_url, after_url)
