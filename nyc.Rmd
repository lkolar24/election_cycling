...
citi_2016 %>% group_by(stop_day, stop_month, user_type) %>% summarize(n = n())
...
stargazer(mod_2, title="Regression Results",
          dep.var.labels=c(""),
...
citi_eday_comparison %>% filter(year == 2014) %>% 
  ggplot(aes(x = avg_all, y = stops_all_eday)) + geom_point() + xlim(0, 400) + ylim(0, 400) + 
...
citi_eday_comparison %>% filter(year == 2018) %>% 
  ...
  geom_histogram() + geom_vline(xintercept = perc_exp_eday_18)

citi_eday_comparison %>% filter(year == 2018) %>% 
  inner_join(all_station_ids) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  ggplot(aes(x = perc_avg_eday, y = stops_all_eday, color = type)) + 
  geom_point() + geom_vline(xintercept = perc_exp_eday_18)

citi_eday_comparison %>% filter(year == 2018) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  inner_join(all_station_ids) %>% st_as_sf() %>% 
  filter(perc_avg_eday > 0.5, perc_avg_eday < 1) %>% 
  mapview(zcol = "perc_avg_eday")

citi_eday_comparison %>% filter(year == 2018) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  mutate(which = ifelse(perc_avg_eday < perc_exp_eday_18 + 0.1,
                        ifelse(perc_avg_eday > perc_exp_eday_18 - 0.1, 
                               "near", "low"), "high")) %>% 
...
citi_eday_comparison %>% filter(year == 2014) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  ggplot(aes(x = perc_avg_eday)) + 
  geom_histogram() + geom_vline(xintercept = perc_exp_eday_14)

citi_eday_comparison %>% filter(year == 2014) %>% 
  inner_join(all_station_ids) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  ggplot(aes(x = perc_avg_eday, y = stops_all_eday, color = type)) + 
  geom_point() + geom_vline(xintercept = perc_exp_eday_14)
...
citi_eday_comparison %>% filter(year == 2014) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  mutate(which = ifelse(perc_avg_eday < perc_exp_eday_14 + 0.1,
                        ifelse(perc_avg_eday > perc_exp_eday_14 - 0.1, 
                               "near", "low"), "high")) %>% 
  inner_join(all_station_ids) %>% st_as_sf() %>% mapview(zcol = "which")

perc_avgs_eday_2014 <- (citi_eday_comparison %>% 
  filter(year == 2014) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all))$perc_avg_eday

perc_avgs_eday_2018 <- (citi_eday_comparison %>% 
  filter(year == 2018) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all))$perc_avg_eday

c(mean(perc_avgs_eday_2014) - 1.96*(sd(perc_avgs_eday_2014)/sqrt(length(perc_avgs_eday_2014))), 
  mean(perc_avgs_eday_2014) + 1.96*(sd(perc_avgs_eday_2014)/sqrt(length(perc_avgs_eday_2014))))

c(mean(perc_avgs_eday_2018) - 1.96*(sd(perc_avgs_eday_2014)/sqrt(length(perc_avgs_eday_2014))), 
  mean(perc_avgs_eday_2018) + 1.96*(sd(perc_avgs_eday_2014)/sqrt(length(perc_avgs_eday_2014))))
...
citi_2018_eday_distances %>% 
  lm(perc_avg_eday ~ n_within_250m, data = .) %>% summary()

citi_2018_eday_distances %>% 
  lm(perc_avg_eday ~ sqrt(min_polling_distance), data = .) %>% summary()
...
####
combined_ids_polling %>% filter(type == "polling")
...
nyc_fig1b <- NYC_boundary %>%
  ggplot() + geom_sf(fill = "aliceblue") +  
  geom_sf(data = citi_eday_comparison %>% 
            filter(year == 2018) %>% 
            st_as_sf() %>% 
            mutate(perc_avg_eday = stops_all_eday/avg_all), 
...
nyc_fig7b <- NYC_boundary %>%
  ...
  ggplot() + geom_sf(fill = "aliceblue") +  
  geom_sf(data = citi_2018_eday_distances %>% 
...
nyc_fig6a <- citi_eday_comparison %>% filter(year == 2014) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  ggplot(aes(x = perc_avg_eday)) + 
  geom_histogram() + geom_vline(xintercept = perc_exp_eday_14, lwd = 1, color = "red") + 
  scale_y_continuous(expand = c(0,0)) + 
  labs(y = "", x = "2014") + 
  theme_minimal() + 
  theme(plot.background = element_rect(fill = "white", color = NA))

nyc_fig6b <- citi_eday_comparison %>% filter(year == 2018) %>% 
  mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
  ggplot(aes(x = perc_avg_eday)) + 
  geom_histogram() + geom_vline(xintercept = perc_exp_eday_18, lwd = 1, color = "red") + 
  scale_y_continuous(expand = c(0,0)) + 
  labs(y = "", x = "2018") + 
  theme_minimal() + 
  theme(plot.background = element_rect(fill = "white", color = NA))
...
ggsave("figures/nyc/nyc_fig7.png", width = 8, height = 8, dpi = 300) 
