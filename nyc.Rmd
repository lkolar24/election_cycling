...
conf_int_2018 <- c(mean(perc_avgs_eday_2018) - 1.96*(sd(perc_avgs_eday_2014)/sqrt(length(perc_avgs_eday_2014))), 
...
nyc_fig2 <- NYC_boundary %>%
  ...
  geom_sf(data = NYC_turnout_all %>% st_as_sf(), fill = NA, lwd = 0.1,
          color = "grey") + 
  geom_sf(data = all_station_ids %>% 
            full_join(citi_eday_comparison %>% 
                        mutate(perc_avg_eday = stops_all_eday/avg_all) %>% 
                        as_tibble() %>% 
                        filter(year == 2018) %>% 
                        select(end_station_id, perc_avg_eday)) %>% 
